 ...
 *
 */
public class Allocator<T> implements Iterable<T> {
	private final Map<T, Integer> indices;
	private final List<T> allObjects;
	private final Allocateable<T> tFactory;
	private int numberInUse;
	
	public Allocator(final Allocateable<T> factory) {
		indices = new HashMap<>();
	...
	private static class AllocatorIterator<T> implements Iterator<T> {
		...
		public AllocatorIterator(final Allocator<T> allocator) {
